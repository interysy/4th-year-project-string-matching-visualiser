...
%==============================================================================
%% METADATA
\title{String Search Algorithm Visualisation Software}
...
%     \caption{Brute Force Substring Search}
%     \KwData{%
%         $text$, the text we will be searching \\
%         $pattern$, the pattern we are searching for in the text.
%     }
%     \KwResult{%
%         The result can typically be modified for a specific purpose. Typically, the index of the starting character where the pattern was found in the text will be returned (as $patternIndex$), or $-1$ if there was no match.
%     }

%     \BlankLine
%     \textbf{bruteForce(text, pattern):}

%     \BlankLine
%     $textLength \gets$ length of $T$ \\
%     $patternLength \gets$ length of $P$ \\
%     $startingPoint \gets 0$ \\
%     $textIndex \gets 0$ \\
%     $patternIndex \gets 0$

%     \BlankLine
%     \While{$startingPoint \leq textLength - patternLength$ \textbf{and} $patternIndex < patternLength$}{
%         \BlankLine
%         \If{$T[textIndex] = P[patternIndex]$}{
%             $textIndex \gets textIndex + 1$ \\
%             $patternIndex \gets patternIndex + 1$
%         } \Else {
%             $patternIndex \gets 0$ \\
%             $startingPoint \gets startingPoint + 1$ \\
%             $textIndex \gets startingPoint$
%         }
%     }

%     \BlankLine
%     \If{$patternIndex = patternLength$}{
%         \textbf{Output:} $startingPoint$
%     } \Else {
%         \textbf{Output:} $-1$
%     }
% \end{algorithm}
...
\begin{figure}[hpt]
  ...
  \begin{tikzpicture}
    ...
    % Draw a red border around the specified range (excluding labels)
...
\begin{figure}[hpt]
  ...
  \begin{tikzpicture}
    ...
    \end{scope}
    % Second drawing
    \begin{scope}[yshift=-6cm] % Adjust the yshift value as needed
...
% \begin{figure}
%   \centering
%   \includegraphics[width=0.8\textwidth]{images/attempt2.drawio.png}
...
\begin{figure}
  \centering
  \begin{tikzpicture}
    \filldraw[fill=gray!50] (0,4) rectangle (15,5);
    \filldraw[fill=green!50] (0,4) rectangle (2,5);
    \filldraw[fill=red!50] (2,4) rectangle (3,5);

    \filldraw[fill=gray!50] (0,2) rectangle (5,3);
    \filldraw[fill=green!50] (0,2) rectangle (2,3);
    \filldraw[fill=red!50] (2,2) rectangle (3,3);

    \draw[->] (1.5,2.8) -- node[below] {Matched Prefix} (1.5,4);
    % \draw[->] (2.5,2) -- node[below] {Mismatched Character} (-2,0);
